...
*/
       ...
       */
        ...
        */
        ...
        {
            //rnd = new Random();
            t.Speak(".Olá sou o teu assistente, acêna-me para me conhecêres");
            //return;
        }
        ...
        private void onMessage(dynamic json, float confidence)
        {
            ...
            if (iniciou == false)
            {
                if (Convert.ToDouble(json.Swipe_Right.ToString()) > 0.4)
                {
                    sayDevsInformation();
                    iniciou = true;
                }
            ...
            {
                if (Convert.ToDouble(json.Hands_air.ToString()) > 0.7)
                {
                    if (lazer == false) { 
                        t.Speak(_okMessages[rnd.Next(0, _okMessages.Length)] + Properties.Settings.Default.voiceCallClient
                        + ".Vejo que quer ter o seu momento de lazer"+ ".Súaipe Esquerda para filmes"+ ".Súaipe Direita para musica");
                        lazer = true;
                    }


                }
                else if (Convert.ToDouble(json.Kill.ToString()) > 0.5)
                {
                    if (lazer)
                    {
                        t.Speak(_okMessages[rnd.Next(0, _okMessages.Length)] + Properties.Settings.Default.voiceCallClient
                     + ".hora de trabalhar" + ".Súaipe Esquerda para Email" + ".Súaipe Direita para Navegar");
                ...
                else if (Convert.ToDouble(json.Swipe_Right.ToString()) > 0.4)
                {
                    if (lazer)
                    {
                        startPlayer();
                    ...
                    else
                    {
                        t.Speak(_okMessages[rnd.Next(0, _okMessages.Length)] + Properties.Settings.Default.voiceCallClient
                      + ".A abrir o Browser");
                        Process.Start("www.elearning.ua.pt");
                    }
                }
                else if (Convert.ToDouble(json.Swipe_Left.ToString()) > 0.5)
                {
                    if (lazer)
                    {
                        t.Speak(_okMessages[rnd.Next(0, _okMessages.Length)] + Properties.Settings.Default.voiceCallClient
                    + ".Parece-me o momento ideal para ver um filme");
                ...
                else if (Convert.ToDouble(json.Jarbas_init.ToString()) > 0.5)
                {
                    t.Speak(_okMessages[rnd.Next(0, _okMessages.Length)] + Properties.Settings.Default.voiceCallClient
                    + ". A bloquear a sessão...");
                    LockWorkStation();
                }
                else if (Convert.ToDouble(json.Play_Pause.ToString()) > 0.5)
                {
                    takeScreenshot();
                }
            ...
            */
            ...
            {
                case "kill":
                    Console.WriteLine("KILLLLLLLLLLLLLLLLLLLLL");
                    //sayHello();
                    return;

                case "pray":
                    Console.WriteLine("PRAYYYYYYYYYYYYYYYYYYYYY");
